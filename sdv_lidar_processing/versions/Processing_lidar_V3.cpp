...
void Processing_lidar::pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr segmented_cluster(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr all_clusters(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::search::Search<pcl::PointXYZ>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZ>);
    tree->setInputCloud(input_cloud); // Assuming segmented_clouds.first is the road_cloud
    ...
    ec.setInputCloud(segmented_clouds.first);
    ec.extract(cluster_indices);

    std::vector<BBox> bboxes;
    size_t min_reasonable_size = 310;
    ...
    for (size_t i = 0; i < cluster_indices.size(); ++i) {
        if (cluster_indices[i].indices.size() > min_reasonable_size && cluster_indices[i].indices.size() < max_reasonable_size) {
            pcl::PointCloud<pcl::PointXYZ>::Ptr reasonable_cluster(new pcl::PointCloud<pcl::PointXYZ>);
            pcl::ExtractIndices<pcl::PointXYZ> extract;
            pcl::IndicesPtr indices(new std::vector<int>(cluster_indices[i].indices.begin(), cluster_indices[i].indices.end()));
            extract.setInputCloud(segmented_clouds.first);
            ...
            pcl::getMinMax3D<pcl::PointXYZ>(*reasonable_cluster, min_pt, max_pt);
    ...
    int id = 0;
    const std_msgs::msg::Header& inp_header = msg->header;
